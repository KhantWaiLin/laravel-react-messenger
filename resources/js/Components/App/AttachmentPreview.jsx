...
import { formatBytesName, isPDF, isPreviewable } from "@/helpers";
...
const AttachmentPreview = ({ file }) => {
    return (
        <div className="w-full flex items-center gap-2 py-2 px-3  rounded-md bg-slate-800">
            <div>
                {isPDF(file.file) && (
                    <img src={"/img/pdf.png"} alt={file.name} className="w-8" />
                )}
            ...
            <div className="flex-1 text-gray-400 text-nowrap text-ellipsis overflow-hidden">
                ...
                <p className="text-xs">{formatBytesName(file.file.size)}</p>
            </div>
        </div>
    );
};
